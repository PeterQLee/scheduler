#!/usr/bin/env python3
###Find Courses according to the criteria the user has specified###
...
if validcook:
    ...
    allCourses=list(DB_conn.find_course({"tag":{"$in":tag},"season":srchseas}))
    
    
    #ty=g.Users

    userCourses=list(DB_conn.find_user({"email":curemail})["courses"])

    allCourses+=(list(DB_conn.find_course({"_id":{"$in":userCourses}})))
